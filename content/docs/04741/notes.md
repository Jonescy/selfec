...
1. 检错重发
   - 停-等协议和滑动窗口协议实现的都是这类差错控制方式
2. 向前纠错
   - 比较适用于单工链路或对实时性要求比较高的应用
3. 反馈校验
   - 优点是原理简单，易于实现，无须差错编码
   - 缺点是需要同时传输能力的反向信道，传输效率低，实时性差
4. 检错丢弃
   - 只适用于实时性要求较高的系统
...
- 发送端以低于信道容量C的数据速率R发送消息，则一定存在一种编码方式。
- 使得误码率P随着码长n的增加而按指数下降至任意小的值。
- 理论上可以通过编码使得数据传输过程中不发生错误，或者将错误概率控制在很小的数值之下。
...
- 汉明距离：
  - 两个等长码字之间，对应位不同的位数。记为d~c~。
  - 定义一个可编码的汉明距离为该编码集中任意两个码字之间汉明距离的最小值，记为d~s~。

不同的差错编码的检错或纠错能力不同。差错编码的检错或纠错能力与编码集的**汉明距离**有关。[课后习题3]
...
1. 信道划分MAC 协议：时分多路复用(time division multiple access)
...
1. 信道划分MAC 协议：频分多路复用(frequency division multiple access)
...
- 信道划分MAC协议：波分多路复用
- 广泛应用在光纤通信中，本质上是一种频分多路复用，由于在光纤通信中，光载波频率很高，所以通常使用波长来代替频率
...
1. **载波监听多路访问协议CSMA** (carrier sense multiple access)，“先听后说”
2. 发送帧之前，监听信道 (载波)：
   - 信道空闲：发送完整帧
   - 信道忙：推迟发送
3. 细分为3种不同类型的CSMA协议
...
1. 带冲突检测的载波监听多路复用访问协议: CSMA with Collision Detection
...
2. 冲突检测:
   ...
   - 无线局域网很难实现：接收信号强度淹没在本地发射信号强度下 **“边发边听，不发不听”**
...
1. **高级数据链路控制协议：HDLC** 工作在同步串行链路上，是一种面向比特的链路层协议；不支持异步链路和点到多点连接，不支持验证和IP地址自动协商。HDLC协议的特点：
...
1. 数据通信系统(模型)的构成：
   ...
   2. 发送设备：将信源产生的信号进行适当的变换的装置，使之能够在信道中传输。变换的方式主要包括编码和调制。
   3. 信道：信号传输的媒介，分为两大类：
      1. 有线信道
         1. 双绞线
         2. 同轴电缆
      ...
      2. 无线信道
         1. 大气层
         2. 外层空间等。
   4. 接收设备：完成发送设备的反变换，即译码和解调。
   5. 信宿：信号的终点，并将信号转换为人性化
   6. 噪声：对通信信号进行感染和影响的各种信号
2. 模拟通信和数字通信
   1. 模拟信号：因变量连续的。
   2. 数字信号：因变量离散的。
3. 通信方式
...
2. 数字基带传输码型
   ...
   <img src="../../../static/img/04741/image-20210506172401524.png" alt="image-20210506172401524" style="zoom: 67%;" /><img src="计算机网络-韩立刚.assets/image-20210506172338644.png" alt="image-20210506172338644" style="zoom:80%;" />
...
- ISO术语的两种设备：
  ...
  - 数据终端设备(Data Terminating Equipment,DTE)，如：计算机。

  - 数据电路端设备(Data Circuit-Terminating Equipment,DCE)，如：调制解调器。
...
![img](../../../static/img/04741/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80MDYyOTI0NA==,size_16,color_FFFFFF,t_70-16341830375591.png)
...
![img](../../../static/img/04741/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80MDYyOTI0NA==,size_16,color_FFFFFF,t_70-16341830408933.png)
...
![img](../../../static/img/04741/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80MDYyOTI0NA==,size_16,color_FFFFFF,t_70-16341830437285.png)
...
![在这里插入图片描述](../../../static/img/04741/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3N0YXJ0ZXJfX19fXw==,size_16,color_FFFFFF,t_70.png)
...
![在这里插入图片描述](../../../static/img/04741/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM5MzQ2NTM0,size_16,color_FFFFFF,t_70%23pic_center.png)
